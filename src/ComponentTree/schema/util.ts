...
export function schemaConverter(
  ...
  return map(newSchema, converter, true);
}

// 默认节点转换函数
export const DEFAULT_CONVERTER_TO_NEW = function(node: any) {
  const newNode = Object.assign({}, node);
  newNode.name = node.name || node.component;
  newNode.component = {
    name: newNode.name
...
// 将旧版转换成新版
export function schemaConvertToNew(
  schema: any,
  converter = DEFAULT_CONVERTER_TO_NEW
): any {
  return map(schema, converter, true);
}
